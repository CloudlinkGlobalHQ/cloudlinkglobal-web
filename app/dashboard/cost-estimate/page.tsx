...
export default function CostEstimatePage() {
  ...
  const confidenceColor = (c: string) => {
    ...
    return 'bg-[#1A2340] text-slate-600'
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-100">Pre-Deploy Cost Estimation</h1>
      ...
      {/* Service selector */}
      <div className="bg-[#0F1629] border border-[#1E2D4F] rounded-xl p-6 shadow-sm">
        <h2 className="text-sm font-semibold text-slate-100 mb-3">Select a service to estimate</h2>
        <div className="flex gap-3">
          <select value={service} onChange={e => setService(e.target.value)}
            className="flex-1 px-3 py-2.5 border border-[#1E2D4F] rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#0F1629] border border-[#1E2D4F] rounded-xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-100">Estimated Impact: {estimate.service}</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-[#141C33] rounded-lg p-4">
                ...
                <p className="text-xl font-bold text-slate-100">${estimate.baseline_monthly_usd?.toLocaleString()}</p>
              ...
              <div className="bg-[#141C33] rounded-lg p-4">
                ...
                <p className="text-xl font-bold text-slate-100">${estimate.estimated_post_deploy_monthly_usd?.toLocaleString()}</p>
          ...
          {estimate.budget_impact && estimate.budget_impact.length > 0 && (
            <div className="bg-[#0F1629] border border-[#1E2D4F] rounded-xl p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-100 mb-3">Budget Impact</h3>
              <div className="space-y-3">
                {estimate.budget_impact.map((bi, i) => (
                  <div key={i} className={`rounded-lg p-4 ${bi.would_breach ? 'bg-red-50 border border-red-200' : 'bg-[#141C33]'}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-slate-100">{bi.budget_name}</p>
