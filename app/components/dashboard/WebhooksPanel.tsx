    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowForm(s => !s)} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">+ Add webhook</button>
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {ALL_EVENTS.map(ev => (
                <label key={ev} className="flex items-center gap-1.5 cursor-pointer">
                  <input type="checkbox" checked={form.events.includes(ev)} onChange={() => toggleEvent(ev)} className="rounded text-indigo-600" />
          ...
          <div className="flex gap-3">
            <button onClick={handleAdd} disabled={saving || !form.url} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
            ...
            <tbody className="divide-y divide-slate-100">
              {hooks.map((h: any) => (
                <tr key={h.webhook_id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><div className="flex flex-wrap gap-1">{(h.events || []).map((ev: string) => <span key={ev} className="text-xs bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded font-mono">{ev}</span>)}</div></td>
