        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {runs.map(r => (
                <tr key={r.run_id} className="hover:bg-[#141C33]">
                  ...
                  <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${r.status === 'SUCCESS' ? 'bg-green-900/40 text-green-300' : 'bg-[#1A2340] text-slate-400'}`}>{r.status || 'done'}</span></td>
