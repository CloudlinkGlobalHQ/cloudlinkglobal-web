    ...
    <div>
      ...
      {showPolicies && (
        <div className="bg-[#0F1629] border border-[#1E2D4F] rounded-xl p-6 mb-6">
          <h2 className="font-semibold text-slate-100 mb-1">Approval policies</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {ACTION_TYPES.map(type => {
              ...
              return (
                <div key={type} className="flex items-center justify-between border border-[#1E2D4F] rounded-lg px-4 py-3">
                  <div>
                  ...
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-[#0F1629] transition-transform ${requiresApproval ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
              )
            })}
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {FILTERS.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filter === f ? 'bg-green-600 text-white' : 'bg-[#0F1629] border border-[#1E2D4F] text-slate-400 hover:border-[#10B981]/50'}`}>
            {f.replace(/_/g, ' ')}
          </button>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {actions.map(a => {
                ...
                return (
                  <>
                    <tr key={a.action_id} className="hover:bg-[#141C33] cursor-pointer" onClick={() => setExpanded(isOpen ? null : a.action_id)}>
                      <td className="px-4 py-3"><span className="font-medium text-slate-200">{a.action_type?.replace(/_/g, ' ')}</span></td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[a.status] || 'bg-[#1A2340] text-slate-400'}`}>
                    ...
                    {isOpen && (
                      <tr key={`${a.action_id}-detail`} className="bg-[#141C33]">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-slate-600">
                            ...
                            {a.proposed_change && Object.keys(a.proposed_change).length > 0 && (
                              <div className="md:col-span-4">
                                ...
                                <pre className="bg-[#0A0E1A] border border-[#1E2D4F] rounded p-2 text-xs overflow-auto">{JSON.stringify(a.proposed_change, null, 2)}</pre>
                              </div>
                            )}
                          </div>
                        </td>
